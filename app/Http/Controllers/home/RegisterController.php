...
class RegisterController extends Controller
{
    ...
    */
    ...
    {
        return view('home/login/Pregister');
    }
    ...
    */
    public function upDate(Request $request)
    {
        ...
        // 如果注册失败返回注册页
        if($validator->fails()){
            return redirect('home/phonereg')->withErrors($validator)->withInput();
        }

        // 3. 验证用户手机号是否存在
        $user = User::where('phone',$input['phone'])->first();
        if (!empty($user)) {
             return redirect('home/login')->with('errors','您的手机号已经注册过了,请前往登录');
        }

        // 判断两次密码是否一致
        if($input['password'] != $input['repass']) {
            return redirect('home/phonereg')->with('errors','两次密码不一致');
        }

        // 判断手机验证码是否正确
        if($input['code'] != session('phonecode')) {
            return redirect('home/phonereg')->with('errors','验证码不正确');
        }
		...
		$do = array("的","爱","咬","抓","给","扯","和","用","绿","打","就","迎","向","踢","笑","闻","有","等于","保卫","演变");
        ...
        if($res){
           ...
           return redirect('home/phonereg')->with('errors','注册失败请重新注册');
        }
    ...
    */
    public function sendCode(Request $request)
    {
        ...
        $template = new SendTemplateSMS();
        $result = new M3Result; 
        $result= $template->sendTemplateSMS($phone,$arr,1);
        // 返回给客户端的json格式数据
